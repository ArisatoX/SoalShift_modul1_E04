...
`awk '(!(/[sS][uU][dD][oO]/) && (/[cC][rR][oO][nN]/) && (NF<13))' /var/log/syslog > /home/arisatox/modul1`
```

`(!(/[sS][uU][dD][oO]/) && (/[cC][rR][oO][nN]/)` artinya tidak mengandung string sudo tetapi mengandung string cron. Dibuat `sS uU dD oO` karena bersifat tidak case sensitive sehingga bisa menggunakan salah satu dari huruf tersebut. `(NF<13)` artinya jumlah field kurang dari 13. NF merupakan built-in variable untuk menentukan banyaknya field. `/var/log/syslog > /home/arisatox/modul1` artinya memindahkan record tadi ke `/home/[user]/modul1`. 
```bash
2-30/6 * * * *. /bin/bash /path/to/script
```
Untuk crontab menggunakan konfigurasi seperti di atas.
